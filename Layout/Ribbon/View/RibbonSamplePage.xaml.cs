...
namespace SfRibbon.Ribbon
{
    ...
    /// </summary>
    public sealed partial class RibbonSamplePage : Page
    {
        public string sampleContent;
        public FontFamily RibbonFontFamily { get; set; }
        public FontFamily ShapeIconFontFamily { get; set; }
        public FontFamily BackStageFontFamily { get; set; }
        public RibbonSamplePage()
        {
    ...
    "Yu Gothic UI"
            ...
            InstalledFonts = new ObservableCollection<SfRibbonComboBoxItem>();
            foreach (var item in Fonts)
            {
                SfRibbonComboBoxItem _ComboBoxItem = new SfRibbonComboBoxItem();
                _ComboBoxItem.Content = item;
                InstalledFonts.Add(_ComboBoxItem);
            }

            int[] size = { 8, 9, 10, 11, 12, 13, 14, 15, 16, 18, 20, 22, 24, 26, 28, 32, 74 };
            FontsSize = new ObservableCollection<SfRibbonComboBoxItem>();
            foreach (var item in size)
            {
                SfRibbonComboBoxItem _ComboBoxItem = new SfRibbonComboBoxItem();
                _ComboBoxItem.Content = item;
                FontsSize.Add(_ComboBoxItem);
            }
            RibbonFontFamily = new FontFamily(PathHelper.GetResourcePath("Ribbon/FontIcons/RibbonIcons.ttf#RibbonIcons"));
            ShapeIconFontFamily = new FontFamily(PathHelper.GetResourcePath("Ribbon/FontIcons/Shapes.ttf#Shapes"));
            BackStageFontFamily = new FontFamily(PathHelper.GetResourcePath("Ribbon/FontIcons/BackStageIcons.ttf#BackStageIcons"));
            ...
            if (Picture1Image != null)
                Picture1Image.Source = new BitmapImage(new Uri(PathHelper.GetResourcePath("Ribbon/Assets/Pictures/1.png"), UriKind.RelativeOrAbsolute));
            if (Picture2Image != null)
                Picture2Image.Source = new BitmapImage(new Uri(PathHelper.GetResourcePath("Ribbon/Assets/Pictures/2.png"), UriKind.RelativeOrAbsolute));
            if (Picture3Image != null)
                Picture3Image.Source = new BitmapImage(new Uri(PathHelper.GetResourcePath("Ribbon/Assets/Pictures/3.png"), UriKind.RelativeOrAbsolute));
            for (int i = 1; i <= 16; i++)
            {
                pictures.Add(PathHelper.GetResourcePath("Ribbon/Assets/Pictures/" + i.ToString() + ".png"));
            }
        ...
        private async void CommandExcuted(object param)
        {
            ...
            {
                if (Windows.System.Profile.AnalyticsInfo.VersionInfo.DeviceFamily != "Windows.Mobile" && Clipboard.GetContent().Contains(Windows.ApplicationModel.DataTransfer.StandardDataFormats.Text))
    ...
    private void CollectColors()
        ...
        public static readonly DependencyProperty FontColorsProperty =
            DependencyProperty.Register("FontColors", typeof(ObservableCollection<SolidColorBrush>), typeof(RibbonSamplePage), new PropertyMetadata(null));



        public ObservableCollection<SfRibbonComboBoxItem> FontsSize
        {
            get { return (ObservableCollection<SfRibbonComboBoxItem>)GetValue(FontsSizeProperty); }
            set { SetValue(FontsSizeProperty, value); }
        }

        // Using a DependencyProperty as the backing store for MyProperty.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty FontsSizeProperty =
            DependencyProperty.Register("FontsSize", typeof(ObservableCollection<SfRibbonComboBoxItem>), typeof(RibbonSamplePage), new PropertyMetadata(null));



        public ObservableCollection<SfRibbonComboBoxItem> InstalledFonts
        {
            get { return (ObservableCollection<SfRibbonComboBoxItem>)GetValue(InstalledFontsProperty); }
            set { SetValue(InstalledFontsProperty, value); }
        }

        // Using a DependencyProperty as the backing store for MyProperty.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty InstalledFontsProperty =
            DependencyProperty.Register("InstalledFonts", typeof(ObservableCollection<SfRibbonComboBoxItem>), typeof(RibbonSamplePage), new PropertyMetadata(null));

        private void Redbutton_Click(object sender, RoutedEventArgs e)
        {
            if(sender is SfRibbonButton)
            this.pencilbutton.Foreground = this.budgetbutton.Foreground = this.brushbutton.Foreground = (sender as SfRibbonButton).Foreground;
        }
        ...
        {
            if (this.mainribbon.RibbonState == RibbonState.Hide)
                this.mainribbon.RibbonState = RibbonState.Normal;
            else if (this.mainribbon.RibbonState == RibbonState.Normal)
        ...
        {
            if((sender as SfRibbonComboBox).SelectedItem!=null)
            RichTextContent.FontSize = (int) ((sender as SfRibbonComboBox).SelectedItem as ComboBoxItem).Content;
        }
        ...
        private async void replacebutton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                var dialog = new MessageDialog("Enter the character or word to be replaced.");
                await dialog.ShowAsync();
            }
        }
