...
namespace Syncfusion.SampleBrowser.UWP.SfChart3D
{
    public sealed partial class Area3D : SampleLayout
    {
        public Area3D()
        {
            ...
            AreaChart3D.Rotation = 30;
            if (AnalyticsInfo.VersionInfo.DeviceFamily == "Windows.Mobile")
            {
                AreaChart3D.Margin = new Thickness(10);
            }
            else
            {
                AreaChart3D.Margin = new Thickness(70,20,75,25);
            }
        }
        ...
        public override void Dispose()
        {
            ...
            if (this.AreaChart3D != null)
            {
                foreach (var series in this.AreaChart3D.Series)
                {
                    series.ClearValue(ChartSeriesBase.ItemsSourceProperty);
                }
                this.AreaChart3D = null;
            }
