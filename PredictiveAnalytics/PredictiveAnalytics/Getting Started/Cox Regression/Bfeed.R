...
trainData=bfeed[1:741,]
testData=bfeed[742:927,]

# Applying Cox Regression Model to predict Survival
bfeed_Cox = coxph(Surv(duration,delta)~race+poverty+smoke+alcohol+agemth+ybirth+yschool+pc3mth, trainData)
summary(bfeed_Cox)
...
# Load Original "bfeed" dataset from "KMsurv" package
data(bfeed)

# Applying Cox Regression Model to predict Survival
bfeed_Cox_Entire = coxph(Surv(duration,delta)~race+poverty+smoke+alcohol+agemth+ybirth+yschool+pc3mth, bfeed)
summary(bfeed_Cox_Entire)

# Applying Cox Regression model to entire dataset and save the results in a CSV file
bfeedEntirePrediction = predict(bfeed_Cox_Entire, type = "expected", bfeed)

# PMML generation
pmmlFile<-pmml(bfeed_Cox_Entire, data=bfeed)
