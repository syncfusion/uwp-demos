...
breastCancer$cens <- factor(breastCancer$cens, ordered = FALSE)
...
CrossTable(breastCancerTestPrediction, testData$cens,
...
pred <- prediction(labels = testData$cens, predictions = breastCancerTestProbabilities[,2])
...
confusionMatrix(breastCancerTestPrediction,testData$cens, positive = "1")


# PMML generation 
saveXML(pmml(breastCancerTrainingModel, predictedField = "cens"), "BreastCancer.pmml")
...
# Considering integer variable as factor
breastCancerOriginal$cens <- factor(breastCancerOriginal$cens, ordered = FALSE)
