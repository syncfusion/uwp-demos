...
tips_Regression<-lm(tip~.,data=trainData)	 								 					 

# Display summary of Regression Model with accuracy(Adjusted R-squared value- 0.4583 or 46%) of training data 
summary(tips_Regression)

# Predict "price" column for test data set
tipsTestPrediction<-predict(tips_Regression,testData)
# Display predicted values
tipsTestPrediction


# PMML generation
pmmlFile<-pmml(tips_Regression,data=trainData)
...
# Load Original "tips" dataset from "reshape2" package
data(tips)
# Omit rows with missing values
tipsOriginal = na.omit(tips)

# Applying Regression model to entire dataset and save the results in a CSV file
tipsEntirePrediction<-predict(tips_Regression,tipsOriginal)
