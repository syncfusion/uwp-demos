...
namespace EssentialDocIO
{
    ...
    /// </summary>
    public sealed partial class HTMLToWord : SampleLayout
    {
        public override void Dispose()
        {
            ...
            HTMLToWordSample.ClearValue(Grid.BackgroundProperty);
            HTMLToWordSample.ClearValue(Grid.PaddingProperty);
            HTMLToWordSample.Children.Clear();
            HTMLToWordSample.ColumnDefinitions.Clear();
            HTMLToWordSample.RowDefinitions.Clear();
            HTMLToWordSample = null;
        }
        ...
        {
            Assembly execAssm = typeof(HTMLToWord).GetTypeInfo().Assembly;
            Stream inputStream = execAssm.GetManifestResourceStream("Syncfusion.SampleBrowser.UWP.DocIO.DocIO.Assets.HTMLToWord.html");
        ...
        public HTMLToWord()
        {
            ...
            {
                HTMLToWordSample.Margin = new Thickness(10, 10, 10, 10);
        ...
        {
            Assembly execAssem = typeof(HTMLToWord).GetTypeInfo().Assembly;
            //Load an existing Word document.
            WordDocument document = new WordDocument();
            Stream inputStream = execAssem.GetManifestResourceStream("Syncfusion.SampleBrowser.UWP.DocIO.DocIO.Assets.HTMLToWord.html");
            // Open an existing template document.
            await document.OpenAsync(inputStream, FormatType.Html);
            inputStream.Dispose();
            SaveDocx(document);
        }
        /// <summary>
        ...
        private async void SaveDocx(WordDocument doc)
        {
            ...
            if (!(Windows.Foundation.Metadata.ApiInformation.IsTypePresent("Windows.Phone.UI.Input.HardwareButtons")))
            {
                ...
                savePicker.SuggestedFileName = "HTMLToWord";
                stgFile = await savePicker.PickSaveFileAsync();
            }
            else
            {
                StorageFolder local = Windows.Storage.ApplicationData.Current.LocalFolder;
                stgFile = await local.CreateFileAsync("HTMLToWord.docx", CreationCollisionOption.ReplaceExisting);
            }
            if (stgFile != null)
            {
                MemoryStream stream = new MemoryStream();
        ...
        private async void ViewTemplate(MemoryStream stream)
        {
            ...
            {
                StorageFolder local = Windows.Storage.ApplicationData.Current.LocalFolder;
                stgFile = await local.CreateFileAsync("HTMLToWord.html", CreationCollisionOption.ReplaceExisting);
            }
            if (stgFile != null)
            {
                stream.Position = 0;
