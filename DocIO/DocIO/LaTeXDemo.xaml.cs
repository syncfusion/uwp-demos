...
namespace EssentialDocIO
{
    ...
    /// </summary>
    public sealed partial class LaTeXDemo : SampleLayout
    {
        public override void Dispose()
        {
            ...
            LaTeX.ClearValue(Grid.BackgroundProperty);
            LaTeX.ClearValue(Grid.PaddingProperty);
            LaTeX.Children.Clear();
            LaTeX.ColumnDefinitions.Clear();
            LaTeX.RowDefinitions.Clear();
            LaTeX = null;
        }
        ...
        private async void SaveDocx(WordDocument doc)
        {
            ...
            if (!(Windows.Foundation.Metadata.ApiInformation.IsTypePresent("Windows.Phone.UI.Input.HardwareButtons")))
            {
                ...
                savePicker.SuggestedFileName = "LaTeXToMath";
                stgFile = await savePicker.PickSaveFileAsync();
            }
            else
            {
                StorageFolder local = Windows.Storage.ApplicationData.Current.LocalFolder;
                stgFile = await local.CreateFileAsync("LaTeXToMath.docx", CreationCollisionOption.ReplaceExisting);
            }
            if (stgFile != null)
            {
