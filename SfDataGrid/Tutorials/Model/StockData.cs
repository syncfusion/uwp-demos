using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace DataGrid
{
    /// <summary>
    /// This class represents the StockData
    /// </summary>
    public class StockData : INotifyPropertyChanged
    {
        #region Private Members

        private string symbol;
        private double trade1;
        private double trade2;
        private double trade3;
        private double trade4;
        private double trade5;
        private double trade6;
        private double trade7;
        private double trade8;
        private double trade9;
        private double trade10;
        private double trade11;
        private double trade12;
        private double trade13;
        private double trade14;
        private double trade15;
        private double trade16;
        private double trade17;
        private double trade18;
        private double trade19;
        private double trade20;
        private double trade21;
        private double trade22;
        private double trade23;
        private double trade24;
        private double trade25;
        private double trade26;
        private double trade27;
        private double trade28;
        private double trade29;

        #endregion

        /// <summary>
        /// Gets or sets the Symbol.
        /// </summary>
        public string Symbol
        {
            get
            {
                return symbol;
            }
            set
            {
                symbol = value;
                RaisePropertyChanged("Symbol");
            }
        }

        /// <summary>
        /// Gets or sets the Trade1.
        /// </summary>
        public double Trade1
        {
            get
            {
                return trade1;
            }
            set
            {
                trade1 = value;
                RaisePropertyChanged("Trade1");
            }
        }

        /// <summary>
        /// Gets or sets the Trade2.
        /// </summary>
        public double Trade2
        {
            get
            {
                return trade2;
            }
            set
            {
                trade2 = value;
                RaisePropertyChanged("Trade2");
            }
        }

        /// <summary>
        /// Gets or sets the Trade3.
        /// </summary>
        public double Trade3
        {
            get
            {
                return trade3;
            }
            set
            {
                trade3 = value;
                RaisePropertyChanged("Trade3");
            }
        }

        /// <summary>
        /// Gets or sets the Trade4.
        /// </summary>
        public double Trade4
        {
            get
            {
                return trade4;
            }
            set
            {
                trade4 = value;
                RaisePropertyChanged("Trade4");
            }
        }

        /// <summary>
        /// Gets or sets the Trade5.
        /// </summary>
        public double Trade5
        {
            get
            {
                return trade5;
            }
            set
            {
                trade5 = value;
                RaisePropertyChanged("Trade5");
            }
        }

        /// <summary>
        /// Gets or sets the Trade6.
        /// </summary>
        public double Trade6
        {
            get
            {
                return trade6;
            }
            set
            {
                trade6 = value;
                RaisePropertyChanged("Trade6");
            }
        }

        /// <summary>
        /// Gets or sets the Trade7.
        /// </summary>
        public double Trade7
        {
            get
            {
                return trade7;
            }
            set
            {
                trade7 = value;
                RaisePropertyChanged("Trade7");
            }
        }

        /// <summary>
        /// Gets or sets the Trade8
        /// </summary>
        public double Trade8
        {
            get
            {
                return trade8;
            }
            set
            {
                trade8 = value;
                RaisePropertyChanged("Trade8");
            }
        }

        /// <summary>
        /// Gets or sets the Trade9
        /// </summary>
        public double Trade9
        {
            get
            {
                return trade9;
            }
            set
            {
                trade9 = value;
                RaisePropertyChanged("Trade9");
            }
        }

        /// <summary>
        /// Gets or sets the Trade10.
        /// </summary>
        public double Trade10
        {
            get
            {
                return trade10;
            }
            set
            {
                trade10 = value;
                RaisePropertyChanged("Trade10");
            }
        }

        /// <summary>
        /// Gets or sets the Trade11.
        /// </summary>
        public double Trade11
        {
            get
            {
                return trade11;
            }
            set
            {
                trade11 = value;
                RaisePropertyChanged("Trade11");
            }
        }

        /// <summary>
        /// Gets or sets the Trade12.
        /// </summary>
        public double Trade12
        {
            get
            {
                return trade12;
            }
            set
            {
                trade12 = value;
                RaisePropertyChanged("Trade12");
            }
        }

        /// <summary>
        /// Gets or sets the Trade13
        /// </summary>
        public double Trade13
        {
            get
            {
                return trade13;
            }
            set
            {
                trade13 = value;
                RaisePropertyChanged("Trade13");
            }
        }

        /// <summary>
        /// Gets or sets the Trade14
        /// </summary>
        public double Trade14
        {
            get
            {
                return trade14;
            }
            set
            {
                trade14 = value;
                RaisePropertyChanged("Trade14");
            }
        }

        /// <summary>
        /// Gets or sets the Trade15
        /// </summary>
        public double Trade15
        {
            get
            {
                return trade15;
            }
            set
            {
                trade15 = value;
                RaisePropertyChanged("Trade15");
            }
        }

        /// <summary>
        /// Gets or sets the Trade16
        /// </summary>
        public double Trade16
        {
            get
            {
                return trade16;
            }
            set
            {
                trade16 = value;
                RaisePropertyChanged("Trade16");
            }
        }

        /// <summary>
        /// Gets or sets the Trade17
        /// </summary>
        public double Trade17
        {
            get
            {
                return trade17;
            }
            set
            {
                trade17 = value;
                RaisePropertyChanged("Trade17");
            }
        }

        /// <summary>
        /// Gets or sets the Trade18
        /// </summary>
        public double Trade18
        {
            get
            {
                return trade18;
            }
            set
            {
                trade18 = value;
                RaisePropertyChanged("Trade18");
            }
        }

        /// <summary>
        /// Gets or sets the Trade19.
        /// </summary>
        public double Trade19
        {
            get
            {
                return trade19;
            }
            set
            {
                trade19 = value;
                RaisePropertyChanged("Trade19");
            }
        }

        /// <summary>
        /// Gets or sets the Trade20
        /// </summary>
        public double Trade20
        {
            get
            {
                return trade20;
            }
            set
            {
                trade20 = value;
                RaisePropertyChanged("Trade20");
            }
        }

        /// <summary>
        /// Gets or sets the Trade21
        /// </summary>
        public double Trade21
        {
            get
            {
                return trade21;
            }
            set
            {
                trade21 = value;
                RaisePropertyChanged("Trade21");
            }
        }

        /// <summary>
        /// Gets or sets the Trade22
        /// </summary>
        public double Trade22
        {
            get
            {
                return trade22;
            }
            set
            {
                trade22 = value;
                RaisePropertyChanged("Trade22");
            }
        }

        /// <summary>
        /// Gets or sets the Trade23
        /// </summary>
        public double Trade23
        {
            get
            {
                return trade23;
            }
            set
            {
                trade23 = value;
                RaisePropertyChanged("Trade23");
            }
        }

        /// <summary>
        /// Gets or sets the Trade24
        /// </summary>
        public double Trade24
        {
            get
            {
                return trade24;
            }
            set
            {
                trade24 = value;
                RaisePropertyChanged("Trade24");
            }
        }

        /// <summary>
        /// Gets or sets the Trade25
        /// </summary>
        public double Trade25
        {
            get
            {
                return trade25;
            }
            set
            {
                trade25 = value;
                RaisePropertyChanged("Trade25");
            }
        }

        /// <summary>
        /// Gets or sets the Trade26
        /// </summary>
        public double Trade26
        {
            get
            {
                return trade26;
            }
            set
            {
                trade26 = value;
                RaisePropertyChanged("Trade26");
            }
        }

        /// <summary>
        /// Gets or sets the Trade27
        /// </summary>
        public double Trade27
        {
            get
            {
                return trade27;
            }
            set
            {
                trade27 = value;
                RaisePropertyChanged("Trade27");
            }
        }

        /// <summary>
        /// Gets or sets the Trade28
        /// </summary>
        public double Trade28
        {
            get
            {
                return trade28;
            }
            set
            {
                trade28 = value;
                RaisePropertyChanged("Trade28");
            }
        }

        /// <summary>
        /// Gets or sets the Trade29
        /// </summary>
        public double Trade29
        {
            get
            {
                return trade29;
            }
            set
            {
                trade29 = value;
                RaisePropertyChanged("Trade29");
            }
        }

        public void InitializeOn(StockData other)
        {
            this.Symbol = other.Symbol;
            this.Trade1 = other.Trade1;
            this.Trade2 = other.Trade2;
            this.Trade3 = other.Trade3;
            this.Trade4 = other.Trade4;
            this.Trade5 = other.Trade5;
            this.Trade6 = other.Trade6;
            this.Trade7 = other.Trade7;
            this.Trade8 = other.Trade8;
            this.Trade9 = other.Trade9;
            this.Trade10 = other.Trade10;
            this.Trade11 = other.Trade11;
            this.Trade12 = other.Trade12;
            this.Trade13 = other.Trade13;
            this.Trade14 = other.Trade14;
            this.Trade15 = other.Trade15;
            this.Trade16 = other.Trade16;
            this.Trade17 = other.Trade17;
            this.Trade18 = other.Trade18;
            this.Trade19 = other.Trade19;
            this.Trade20 = other.Trade20;
            this.Trade21 = other.Trade21;
            this.Trade22 = other.Trade22;
            this.Trade23 = other.Trade23;
            this.Trade24 = other.Trade24;
            this.Trade25 = other.Trade25;
            this.Trade26 = other.Trade26;
            this.Trade27 = other.Trade27;
            this.Trade28 = other.Trade28;
            this.Trade29 = other.Trade29;
        }

        public event PropertyChangedEventHandler PropertyChanged;

        private void RaisePropertyChanged(string name)
        {
            if (PropertyChanged != null)
                PropertyChanged(this, new PropertyChangedEventArgs(name));
        }
    }

    /// <summary>
    /// This class represents the PerformanceStockData
    /// </summary>
    public class PerformanceStockData : INotifyPropertyChanged
    {
        #region Private Members

        private string symbol;
        private string account;
        private double lastTrade;
        private double change;
        private double previousClose;
        private double open;
        private long volume;

        #endregion

        /// <summary>
        /// Gets or sets the symbol.
        /// </summary>
        /// <value>The symbol.</value>
        public string Symbol
        {
            get
            {
                return symbol;
            }
            set
            {

                symbol = value;
                RaisePropertyChanged("Symbol");
            }
        }

        /// <summary>
        /// Gets or sets the account.
        /// </summary>
        /// <value>The account.</value>
        public string Account
        {
            get
            {
                return account;
            }
            set
            {

                account = value;
                RaisePropertyChanged("Account");
            }
        }

        /// <summary>
        /// Gets or sets the last trade.
        /// </summary>
        /// <value>The last trade.</value>
        public double LastTrade
        {
            get
            {
                return lastTrade;
            }
            set
            {

                lastTrade = value;
                RaisePropertyChanged("LastTrade");
            }
        }

        /// <summary>
        /// Gets or sets the change.
        /// </summary>
        /// <value>The change.</value>
        public double Change
        {
            get
            {
                return change;
            }
            set
            {

                change = value;
                RaisePropertyChanged("Change");
            }
        }

        /// <summary>
        /// Gets or sets the previous close.
        /// </summary>
        /// <value>The previous close.</value>
        public double PreviousClose
        {
            get
            {
                return previousClose;
            }
            set
            {

                previousClose = value;
                RaisePropertyChanged("PreviousClose");
            }
        }

        /// <summary>
        /// Gets or sets the open.
        /// </summary>
        /// <value>The open.</value>
        public double Open
        {
            get
            {
                return open;
            }
            set
            {

                open = value;
                RaisePropertyChanged("Open");
            }
        }

        /// <summary>
        /// Gets or sets the volume.
        /// </summary>
        /// <value>The volume.</value>
        public long Volume
        {
            get
            {
                return volume;
            }
            set
            {

                volume = value;
                RaisePropertyChanged("Volume");
            }
        }

        /// <summary>
        /// Initializes the on.
        /// </summary>
        /// <param name="other">The other.</param>
        public void InitializeOn(PerformanceStockData other)
        {
            this.Symbol = other.Symbol;
            this.LastTrade = other.LastTrade;
            this.Change = other.Change;
            this.PreviousClose = other.PreviousClose;
            this.Open = other.Open;
            this.Volume = other.Volume;
        }

        public event PropertyChangedEventHandler PropertyChanged;

        private void RaisePropertyChanged(string name)
        {
            if (PropertyChanged != null)
                PropertyChanged(this, new PropertyChangedEventArgs(name));
        }
    }
}
