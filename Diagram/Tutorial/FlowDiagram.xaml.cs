        ...
        private void FlowDiagram_ItemAdded(object sender, ItemAddedEventArgs args)
        {
            if(args.Item is CustomNode)
            {
               if((args.Item as CustomNode).Annotations != null)
                {
                    var annotation = (args.Item as CustomNode).Annotations as ObservableCollection<IAnnotation>;
                    foreach (AnnotationEditorViewModel anno in annotation)
                    {                        
                            HorizontalAlignment = HorizontalAlignment.Right;
                            VerticalAlignment = VerticalAlignment.Center;
                            anno.ViewTemplate = XamlReader.Load(FlowDiagramTemplate.vTemplate1) as DataTemplate;
