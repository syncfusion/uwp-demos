...
namespace UMLDiagramDesigner
{
    public partial class Diagram : UserControl
    {
        ...
        PointerEventHandler pointerRelease;

        public Diagram()
        {
            this.InitializeComponent();
            tapped = new TappedEventHandler(Diagram_Tapped);
            pointerRelease = new PointerEventHandler(Diagram_PointerReleased);
            ...
            AddEditType();
            this.AddHandler(TappedEvent, tapped , true);
            this.PointerMoved += Diagram_PointerMoved;
            //this.PointerReleased += Diagram_PointerReleased;
            this.AddHandler(Control.PointerReleasedEvent, pointerRelease, true);
            ...
            this.PointerPressed += Diagram_PointerPressed;
            diagramControl1.Tool = Tool.ZoomPan | Tool.SingleSelect | Tool.MultipleSelect;
        ...
        void Diagram_Unloaded(object sender, RoutedEventArgs e)
        {
            ...
            this.RemoveHandler(TappedEvent, tapped);
            this.RemoveHandler(Control.PointerReleasedEvent, pointerRelease);
        }
        ...
        void diagramControl1_ConnectorTargetChangedEvent(object sender, ChangeEventArgs<object, ConnectorChangedEventArgs> args)
        {
            if (args.NewValue.Node != null)
            {
                ...
                if (line.Content != null)
                {
                    ...
                    {
                        ((line.Segments as ObservableCollection<object>)[0] as StraightSegment).Point = content.SplitPoint;
        ...
        bool ischecktap = true;
        void Diagram_PointerPressed(object sender, PointerRoutedEventArgs e)
        {
        ...
        private static void OnUMLViewModelChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (vm != null)
            {
                ...
                vm.Delete = new DelegateCommand<object>(diagram.OnDelete, diagram.CanDelete);
        ...
        void Diagram_PointerReleased(object sender, PointerRoutedEventArgs e)
        {
        ...
        private void OnAddINodeType(NewNodeType type)
        {
            ...
            center = Root.TransformToVisual(diagramControl1.Page).TransformPoint(center);
        ...
        void Diagram_PointerMoved(object sender, PointerRoutedEventArgs e)
        {
                ...
                properties.IsRightButtonPressed)
            {
                if (IsLineSplitEnabled)
                {
                    ...
                    if (line != null)
                    {
                        Point pos = e.GetCurrentPoint(diagramControl1.Page).Position;
                        ((line.Segments as ObservableCollection<object>)[0] as StraightSegment).Point = pos;
        ...
        void Diagram_Tapped(object sender, TappedRoutedEventArgs e)
        {
        ...
        private void addEditType(EditingType option)
        {
            ...
            nodes.Add(node);
            editType.Add(node);
            node.ManipulationMode = ManipulationModes.None;
        ...
        private void node_PointerMoved(object sender, PointerRoutedEventArgs e)
        {
            if (e.Pointer.IsInContact)
            {
                ...
                if (content.EditType == EditingType.Connect)
                {
                    ...
                    if (this.UMLViewModel.SelectedObject is Node)
                    {
                    WorkFlowEditor.DrawParam param = new WorkFlowEditor.DrawParam(e, this.UMLViewModel.SelectedObject);
            ...
            foreach (var item in editType)
            {
                ...
                switch (content.EditType)
                {
                    case EditingType.Connect:
                        ...
                        (root.Resources["lineanimation"] as Storyboard).Stop();
                        (root.Resources["lineanimation"] as Storyboard).Begin();
            ...
            foreach (var item in editType)
            {
                ...
                switch (content.EditType)
                {
                    ...
                    case EditingType.Split:
                        ...
                        Point center = new Point(bounds.Left + bounds.Width / 2,
                                                 bounds.Top + bounds.Height / 2);
                        center = ((line.Segments as ObservableCollection<object>)[0] as StraightSegment).Point ?? center;
                        Storyboard sb = (item.FindChild<Ellipse>("ellipse").Resources["Storyboard1"] as Storyboard);
                        if (sb.GetCurrentState() == ClockState.Filling || sb.GetCurrentState() == ClockState.Stopped)
                        {
                            sb.RepeatBehavior = new RepeatBehavior() { Type = RepeatBehaviorType.Forever };
                            sb.Begin();
                        }
                ...
                newNode.Loaded += (sender1, event1) =>
                    {
                        ...
                        {
                            animation.RepeatBehavior = new RepeatBehavior() { Type = RepeatBehaviorType.Forever };
                            animation.Begin();
                        }
                        newNode.PointerPressed += (send, evt) =>
                            {
                                animation.Stop();
                            };
                    };
        ...
        private void ShowContextMenu()
        {
            if (UMLViewModel.IsPageEdit)//!diagramControl1.View.IsPageEditable)
            {
                ...
                objectRadialMenu.Visibility = Windows.UI.Xaml.Visibility.Visible;
                ...
                objectRadialMenu.Focus(Windows.UI.Xaml.FocusState.Keyboard);
                objectRadialMenu.IsOpen = true;
        ...
        {
            Point pos = this.TransformToVisual(diagramControl1.Page).TransformPoint(TapPosition);
    ...
    public class SfDiagram1 : SfDiagram
    {
        ...
        {
            return new Connector() { Segments = new ObservableCollection<object> { new StraightSegment() } };
        }
    }
}
