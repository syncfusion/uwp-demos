...
namespace DiagramBuilder.ViewModel
{
    ...
    {
        LabelVM _mAnnotation = new LabelVM() { UnitWidth = 100,Content = " ",UnitHeight = 100, TextHorizontalAlignment = TextAlignment.Center, TextVerticalAlignment = VerticalAlignment.Center };
        ...
        public GroupableVM()
        {
            if (this is NodeVM)
            {
                (this as NodeVM).Annotations = new List<IAnnotation>() { _mAnnotation, _mAnnotationHyperLink };
            }
            else if (this is ConnectorVM)
            {
                (this as ConnectorVM).Annotations = new List<IAnnotation>() { _mAnnotation, _mAnnotationHyperLink };
            }
       ...
       // }
        ...
        public IEnumerable<IAnnotation> AnnotationsE
        {
            get
            {
                if (this is NodeVM)
                {
                    return (this as NodeVM).Annotations as IEnumerable<IAnnotation>;
                }
                else if (this is ConnectorVM)
                {
                    return (this as ConnectorVM).Annotations as IEnumerable<IAnnotation>;
                }
            ...
            switch (name)
            {
                ...
                case "Annotations":
                        ...
                        AnnotationsE.FirstOrDefault() != _mAnnotation))
                    {
                        if (this is NodeVM)
                        {
                            (this as NodeVM).Annotations = new List<IAnnotation>() { _mAnnotation };
                        }
                        else if (this is ConnectorVM)
                        {
                            (this as ConnectorVM).Annotations = new List<IAnnotation>() { _mAnnotation };
                        }
                    }
                    break;
        ...
        private void OnLabelAlignChanged()
        {
            var annotations = new List<IAnnotation>(); ;
            if (this is NodeVM)
            {
                annotations = (this as NodeVM).Annotations as List<IAnnotation>;
            }
            else if (this is ConnectorVM)
            {
                annotations = (this as ConnectorVM).Annotations as List<IAnnotation>;
            }

            foreach (LabelVM item in (annotations as List<IAnnotation>))
            {
    ...
    public class LabelVM : AnnotationEditorViewModel
    {
        ...
        public FontWeight FontWeight
        {
            ...
            set
            {
                //if (_mFontWeight.Weight!= value.Weight)
                {
                    _mFontWeight = value;
                    OnPropertyChanged(GroupableConstants.FontWeight);
                }
