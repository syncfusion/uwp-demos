    ...
    sealed partial class DiagramBuilder : Page
    {
        ...
        private void TabbedDiagrams_Tapped(object sender, TappedRoutedEventArgs e)
        {
            ...
            if (!radialMenu.IsOpen)
            {
                ...
                {
                    double scale = ((this.DataContext as DiagramBuilderVM).SelectedDiagram.Info as IGraph).ScrollSettings.ScrollInfo.CurrentZoom;
        ...
        {
            foreach (DiagramVM diagram in (this.DataContext as DiagramBuilderVM).Diagrams)
            {
                if ((diagram.PortVisibility & PortVisibility.Collapse) == 0)
                {
